...
func (d *Daemon) startAgentHealthHTTPService() {
	...
	for _, host := range hosts {
		...
		if errors.Is(err, unix.EADDRNOTAVAIL) {
			log.Infof("healthz status API server not available on %s", addr)
			available--
			continue
		} else if err != nil {
		...
		go func(addr string, ln net.Listener) {
			...
			if errors.Is(err, http.ErrServerClosed) {
				...
				log.WithError(err).Fatalf("Unable to start healthz status API server on address %s", addr)
			}
		}(addr, ln)
	...
	if available <= 0 {
		log.Fatal("No healthz status API server started")
	}
}
